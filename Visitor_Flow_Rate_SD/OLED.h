...
void LEDPIN_Init(void)    
{
 /* LED_SCL_Init;
  ...
  LED_RST_Init;
  LED_DC_Init;*/
  pinMode(SCL_PIN_SD,OUTPUT);
  pinMode(SDA_PIN_SD,OUTPUT);
...
void LED_WrDat(unsigned char data)   
{
	...
	//LED_SCLL;;; 
        digitalWrite(SCL_PIN_SD,LOW);
	while (i--)
	{
		if (data & 0x80)
		{
			digitalWrite(SDA_PIN_SD,HIGH);;;;
		}
		else
		{
			digitalWrite(SDA_PIN_SD,LOW);;;
		}
		//LED_SCLH;;; 
                digitalWrite(SCL_PIN_SD,HIGH);;;
		asm("nop");;;     
		//LED_SCLL;;;
                digitalWrite(SCL_PIN_SD,LOW);
		data <<= 1;    
